...
interface FileUploadProps {
    onUploadSuccessAction?: (file: File) => void;
    onUploadErrorAction?: (error: FileError) => void;
    acceptedFileTypes?: string[];
    maxFileSize?: number;
    currentFile?: File | null;
    onFileRemoveAction?: () => void;
    /** Duration in milliseconds for the upload simulation. Defaults to 2000ms (2s), 0 for no simulation */
    uploadDelay?: number;
    validateFileAction?: (file: File) => FileError | null;
    className?: string;
}
...
export default function FileUpload({
    ...
    const simulateUpload = useCallback(
        (uploadingFile: File) => {
            ...
            uploadIntervalRef.current = setInterval(() => {
                ...
                if (currentProgress >= 100) {
                    ...
                    setStatus("idle");
                    setFile(null);
                    onUploadSuccessAction?.(uploadingFile);
                } else {
        ...
        [onUploadSuccessAction, uploadDelay]
    );
    ...
    const handleFileSelect = useCallback(
        (selectedFile: File | null) => {
            ...
            const customError = validateFileAction?.(selectedFile);
            if (customError) {
                handleError(customError);
                return;
            }

            // If uploadDelay is 0, we do not simulate any upload. Treat this as a selection-only flow.
            if (uploadDelay === 0) {
                setProgress(0);
                setStatus("idle");
                setFile(null);
                onUploadSuccessAction?.(selectedFile);
                return;
            }
    ...
    const resetState = useCallback(() => {
        ...
        setStatus("idle");
        setProgress(0);
        if (onFileRemoveAction) onFileRemoveAction();
    }, [onFileRemoveAction]);
